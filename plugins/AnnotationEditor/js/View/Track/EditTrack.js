...
var EditTrack = declare(DraggableFeatureTrack,
{
            ...
            stop: function (event, ui) {
                ...
                this.getRefSeq(function (refSeq) {
                    ...
                    this.replaceTranscript(transcript, newTranscript, function (t) {
                        _.each(this.filterExons(t), function (f) {
     ...
     *
     * The return value takes the format:
     * [
     *   // splice pair
     *   [
     *     // donor
     ...
     * ]
     *
     * The first splice pair corresponds to the first intron from the direction
     * the transcript is read in.
     */
    getSpliceSites: function (refSeq, transcript) {
        var junctions = _.flatten(
            this.getCDNACoordinates(transcript, {ignoreStrand: true})
        ).slice(1, -1);
        ...
        if (transcript.get('strand') === -1) {
            _.each(spliceCoordinates, function (pair) {
                pair.reverse();
            });
            spliceCoordinates.reverse();
        }
    ...
    /**
     * Marks non canonical splice sites.
     *
     * The canonical splice donor and acceptor are GT and AG respectively. [1]
     */
    markNonCanonicalSpliceSites: function (transcript, refSeq) {
        ...
        _.each(spliceSites, function (pair) {
            if (pair[0][1].toLowerCase() !== "gt")
                nonCanonicalSpliceSites.push(pair[0]);

            if (pair[1][1].toLowerCase() !== "ag")
                nonCanonicalSpliceSites.push(pair[1]);
        });

        _.each(nonCanonicalSpliceSites, function (site) {
